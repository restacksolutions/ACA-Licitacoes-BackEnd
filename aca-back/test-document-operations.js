...
async function testDocumentOperations() {
  console.log('🔍 Testando operações de documentos (upload, download, exclusão)...');
...
Observações: Documento de teste para verificar upload, download e exclusão
...
4. Permitir exclusão do arquivo`;
    ...
    const testFilePath = path.join(__dirname, 'test-document-operations.pdf');
    ...
    formData.append('file', fs.createReadStream(testFilePath), {
      filename: 'test-document-operations.pdf',
      contentType: 'application/pdf'
    });
    formData.append('clientName', 'Cliente Teste Operações');
    ...
    // 7. Testar exclusão do documento
    ...
    // 8. Verificar se o documento foi removido do banco
    ...
    // 9. Tentar baixar documento excluído (deve falhar)
